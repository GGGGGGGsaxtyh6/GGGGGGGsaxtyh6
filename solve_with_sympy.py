from sympy.ntheory import discrete_log
from Crypto.Util.number import bytes_to_long, long_to_bytes
from functools import reduce

# Parámetros
primes = [
    10124460123717732577,
    12017858281002457601,
    15013023439701145679,
    17297082179958074003,
]

m = 1
for p in primes:
    m *= p
m *= 309938271107370186286857659422407283771865851657937731111744300310040346308690205006641245318085354895438500308814135783504803596134094206083383195557983

A = 246920531455433048826966670968977027196902794022573095007307132382393013927558904839982249967656424317118051446771969623490992917094197975459553923550187
B = 3120090466259654766880909097573553950487818021670727992039931232537173262599219376624291488340607228938989670215080876932583555412966984946786628621917436142053528648425399877486481618672307477414787846984446437445450282569184963
e = 4180488827

output = [263928838896714332657762314348602240455087661491500131260179929152321790693260683004004767933801216877412896467528419672112455669506824987777448696213888416333755744665868795853306879814179767910033263080874331851322593343273834, 7500716958005582985508201577146205078774368134456220390802616323312171951781595852608949532619393759348663684985174625019004577610546554994801687569942118260000292201829011312249563036393035204990811392856155820408283216493092108, 2577794401090299805723177598637436117371525412262771825236813936115237585602114357696686099541164980249580094089526387369627399445447896464668107355247993148990034255262021717192029828447047736912032562575292618672346226630177737, 9097092753543027532933159793556144814276134227405286080166556624316657274280165256861623248902541076900039285592392074751617153907616670295765377162951858063712964557527212039482031296350052367350738141716598584719951567052177519, 8174235910244322855637254130843352735528509191186418212056519911128301630950848710182451421471531481098828227735010211554839205099309290314993113489826162548089590990963249899714956197889959999884447438378454452096405595048394416, 2823506900640363315693941780283568096117331722098156706833610340919194199647098542170689133810904702849664199290565773405646243449194786656181872671323095023235608799444864036190997987871160063941897196360905342203429250068859314, 4625135637721297092934166648990860031642126233341589454312760229225192256970211899532356300747405146834048765116081788846632527090729839758895834854075755594692611781829177188955301784020596520269704429107749955107365380494152652]

known_prefix = b"good luck lmao ictf{"
chunks = [bytes_to_long(known_prefix[i:i+10]) for i in range(0, len(known_prefix), 10)]

target = output[0] ^ chunks[0]

print("Calculando logaritmos discretos módulo cada primo usando sympy...")
print()

logs_mod_primes = []

for i, p in enumerate(primes):
    print(f"Primo {i+1}/{len(primes)}: {p}")
    g = B % p
    h = target % p
    n = p - 1  # orden del grupo
    
    print(f"  Buscando x tal que {g}^x ≡ {h} (mod {p})")
    
    try:
        # sympy.ntheory.discrete_log(n, a, b) resuelve b^x ≡ a (mod n)
        result = discrete_log(p, h, g)
        print(f"  ¡Encontrado! x ≡ {result} (mod {n})")
        logs_mod_primes.append((result, n))
    except Exception as e:
        print(f"  Error: {e}")
        print(f"  Saltando este primo...")

print(f"\n{'='*60}")
print(f"Logaritmos encontrados: {len(logs_mod_primes)}")

if len(logs_mod_primes) >= 2:
    print("\nUsando teorema chino del resto para combinar...")
    
    def chinese_remainder(remainders, moduli):
        total = 0
        prod = reduce(lambda a, b: a*b, moduli)
        for r, mod in zip(remainders, moduli):
            p = prod // mod
            total += r * pow(p, -1, mod) * p
        return total % prod
    
    remainders = [log for log, mod in logs_mod_primes]
    moduli = [mod for log, mod in logs_mod_primes]
    
    combined_x = chinese_remainder(remainders, moduli)
    combined_mod = reduce(lambda a, b: a*b, moduli)
    
    print(f"\nResultado CRT:")
    print(f"x ≡ {combined_x} (mod {combined_mod})")
    print(f"Bits de x: {combined_x.bit_length()}")
    print(f"Bits del módulo: {combined_mod.bit_length()}")
    
    # El verdadero x podría ser combined_x + k * combined_mod
    # Pero como el expon ente es state >> 16 << 16, debería ser múltiplo de 2^16
    
    print(f"\nVerificando si este x funciona...")
    test = pow(B, combined_x, m)
    if test == target:
        print(f"¡VERIFICADO! x = {combined_x}")
        state_found = combined_x
    else:
        print(f"No verifica directamente. Probando x + k * combined_mod...")
        found = False
        for k in range(10000):
            test_x = combined_x + k * combined_mod
            test = pow(B, test_x, m)
            if test == target:
                print(f"¡ENCONTRADO! x = {test_x} (k={k})")
                state_found = test_x
                found = True
                break
            if k % 1000 == 0 and k > 0:
                print(f"  Probado k hasta {k}...")
        
        if not found:
            print("No encontrado en el rango de k probado")
            print("El exponente debe tener más bits que los que cubre el CRT")
            state_found = None
    
    if state_found is not None:
        print(f"\n{'='*60}")
        print(f"ESTADO INICIAL (sin últimos 16 bits) ENCONTRADO!")
        print(f"state_0 & ~0xffff = {state_found}")
        print(f"Bits: {state_found.bit_length()}")
        print()
        
        # Ahora descifrar el mensaje completo
        # Probar con diferentes valores de los últimos 16 bits
        print("Descifrando mensaje...")
        
        for low_bits in range(min(65536, 1000)):  # Probar solo algunos valores inicialmente
            full_state = state_found | low_bits
            
            # Simular el PRNG
            temp_state = full_state
            all_match = True
            
            for i in range(len(chunks)):
                keystream = pow(B, temp_state - (temp_state & 0xffff), m)
                expected = output[i] ^ chunks[i]
                
                if keystream != expected:
                    all_match = False
                    break
                
                # Actualizar estado
                temp_state = pow(pow(A, temp_state, m) ^ B, e, m)
            
            if all_match:
                print(f"¡Los últimos 16 bits son: {low_bits}!")
                print(f"Estado inicial completo: {full_state}")
                
                # Descifrar todo
                plaintext = b""
                temp_state = full_state
                for out in output:
                    keystream = pow(B, temp_state - (temp_state & 0xffff), m)
                    chunk_val = out ^ keystream
                    chunk_bytes = long_to_bytes(chunk_val)
                    plaintext += chunk_bytes
                    temp_state = pow(pow(A, temp_state, m) ^ B, e, m)
                
                print(f"\n{'='*60}")
                print(f"FLAG:")
                print(plaintext.decode('utf-8', errors='ignore'))
                print(f"{'='*60}")
                break
        else:
            print("No se encontraron los últimos 16 bits en el rango probado")
            print("Ampliando búsqueda...")
            # Si no se encuentra, probar todo el rango
            for low_bits in range(1000, 65536):
                full_state = state_found | low_bits
                temp_state = full_state
                keystream0 = pow(B, temp_state - (temp_state & 0xffff), m)
                if keystream0 == (output[0] ^ chunks[0]):
                    # Verificar con el segundo también
                    temp_state = pow(pow(A, temp_state, m) ^ B, e, m)
                    keystream1 = pow(B, temp_state - (temp_state & 0xffff), m)
                    if keystream1 == (output[1] ^ chunks[1]):
                        print(f"¡Encontrado! low_bits = {low_bits}")
                        
                        plaintext = b""
                        temp_state = full_state
                        for out in output:
                            keystream = pow(B, temp_state - (temp_state & 0xffff), m)
                            chunk_val = out ^ keystream
                            chunk_bytes = long_to_bytes(chunk_val)
                            plaintext += chunk_bytes
                            temp_state = pow(pow(A, temp_state, m) ^ B, e, m)
                        
                        print(f"\n{'='*60}")
                        print(f"FLAG:")
                        print(plaintext.decode('utf-8', errors='ignore'))
                        print(f"{'='*60}")
                        break
else:
    print("No se encontraron suficientes logaritmos para usar CRT")
