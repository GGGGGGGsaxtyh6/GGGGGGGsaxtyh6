...
print("=== Resolviendo logaritmo discreto con PARI/GP para cada factor ===\n")
...
for i, p in enumerate(factors):
    print(f"Factor {i+1}/{len(factors)}: p = {p}")
    ...
    x = znlog(Mod(ks_mod, m), Mod(B_mod, m));
    print(x);
    quit;
    """
    
    # Escribir y ejecutar
    with open('/tmp/gp_temp.gp', 'w') as f:
        f.write(gp_script)
    
    result = subprocess.run(['gp', '-q', '/tmp/gp_temp.gp'], 
    ...
    if result.returncode == 0:
        ...
        print(f"  Error ejecutando GP: {result.stderr}")
...
if len(solutions) == len(factors):
    ...
    print("Verificando con m completo...")
    test_ks = pow(B, x_combined, m)
    print(f"B^x mod m == keystream_0: {test_ks == keystream_0}")
    
    if test_ks == keystream_0:
        print("\n¡Verificación exitosa! Ahora bruteforcear los últimos 16 bits...")
        ...
        for low_bits in range(65536):
            ...
            if ks1 == keystreams[1]:
                print(f"\n¡Estado inicial encontrado!: {state_0}")
        ...
        # Probar múltiplos
        print("La verificación directa falló, probando múltiplos...")
        for k in range(100):
            x_test = x_combined + k * mod_combined
            if pow(B, x_test, m) == keystream_0:
                print(f"¡Encontrado con k={k}!: x = {x_test}")
                x_combined = x_test
                break
else:
    print(f"Solo se resolvieron {len(solutions)}/{len(factors)} factores")
